...
public class Player extends Entity{
	...
	public Player(GamePanel gp, keyHandler keyH) {
		...
		this.keyH = keyH; // Makes gp and keyH used in this class equal to those from the GamePanel, or main class		
		
		
		screenX = gp.screenWidth/2 - (gp.tileSize/2); // The substraction fixes the issue that halway og coords are top left of image
...
//		tookDamage = false;
		...
		// 1: A stun wand. Press e or something, and shoots a projectile that does no damage, but locks a monster in place
		// for 5 seconds. Note: If you touch monster, you still get damaged
		// 2: Damage wand. Weaker than a sword, but has a much longer range. Has a cooldown of 1 shot per 3 seconds, so cannot rely
		// solely on want. In addition, due to it's weak strength, will eventually be useless in combat
		// 3: Super damage wand. A much higher damage wand that will deal the largest amout of damage in the game. However, it has
		// a very low cooldown of 12 seconds. In addition, lower range than the damage want. While useful, considering
		// monsters will chase the player often, sword will still be used more than this want. In addition,
		
		// MANA Bar: Under the health bar, mana bar will exist. This mana bar decreases with every cast of a spell. It increases on
		// it's own slowly with time. This is so that spells cannot be spammed and so that swords must be used. 
		
		
		
		// After projectiles, monster projectiles:
		
		// Similar concepts, with one monster stunning player, another doing weak damage, and another very high damage, but slow shot. 
		 ...
		 *  -- Full screen mode
		 *  -- Game over
		 *  -- Monster pathfinding (compare mine to tutorial vid and see how I can improve)
		 *  -- Cutting projectiles and knockback. Especially knockback! Important!
		 *  -- Save files
		 *  -- 4 Maps for now (Tutorial Map, main world, tree maze world, and dirt island world)
		 *  -- 10 different monsters. 5 for each tree maze world and dirt island world. 
		 *  -- For the 5 for each map, 1 basic melee, 1 basic range, 1 wierd (camo/wierd properties), 1 tough enemy (range and melee)
		 *  -- and finally, 1 boss for each. Each boss has 3 attacks. 1-2 melee, 1-2 range. 
		 *  GAME COMPLETED!
		 * 
		 * 
		 * 
...
//		inventory.add(new Object_LeafWand(gp));
	...
	public void getPlayerAttackImage() {
		...
		if (currentWeapon.name == "Basic Sword") {
			...
			// Player attack left and up need adjustment for screen X and Y. They have an extra tile in a certain direction. 
	...
	public void update() {
		if (attacking == true) { // bypass keyInput while attacking
			...
			// Player should be able to move with the code above, but there is something missing. 
			// Since there is no time interval of how often update and repaint repeats, the computer update so fast
			// that y decreases millions of times, due to the number of updates. There must be a time interval that must be added. 
			...
			int objectIndex = gp.cChecker.checkObject(this, true); // this checks for collision, and returns the index of the object that is collided with
	...
	public void attacking() {
		...
		if (attackSpriteCounter > 8 && attackSpriteCounter <= 25) {
			...
			case "UpNone": worldY -= attackArea.height; break;
			case "down": worldY += attackArea.height; break;
			case "DownNone": worldY += attackArea.height; break;
			case "left": worldX -= attackArea.width; break;
			case "LeftNone": worldX -= attackArea.width; break;
			case "right": worldX += attackArea.width; break;
			case "RightNone": worldX += attackArea.width; break;
			}
	...
	public void pickUpObject(int index) {
		...
		if (index != 999) { // only 998 index's total set. If more, can just increase the number. 999 is one out of bounds. 
	...
	public void draw(Graphics2D g2) {
		...
		case "up":
			...
			if (attacking == true) {
				...
				if (spriteNum == 2) {tempScreenY = screenY - gp.tileSize;image = attackUp2;}
			}						
			break;
		case "UpNone":
			if (attacking == false) {
				if (spriteNum == 1) {image = upStand1;}			
				if (spriteNum == 2) {image = upStand2;}
			}
			if (attacking == true) {
				if (spriteNum == 1) {image = attackUp1;}			
				if (spriteNum == 2) {tempScreenY = screenY - gp.tileSize;image = attackUp2;}
		...
		case "left":
			...
			if (attacking == true) {
				...
				if (spriteNum == 2) {tempScreenX = screenX - gp.tileSize;image = attackLeft2;}
			}						
			break;
		case "LeftNone":
			if (attacking == false) {
				if (spriteNum == 1) {image = leftStand1;}			
				if (spriteNum == 2) {image = leftStand2;}
			}
			if (attacking == true) {				
				if (spriteNum == 1) {image = attackLeft1;}			
				if (spriteNum == 2) {tempScreenX = screenX - gp.tileSize;image = attackLeft2;}
		...
		case "RightNone":
			if (attacking == false) {
				if (spriteNum == 1) {image = rightStand1;}			
				if (spriteNum == 2) {image = rightStand2;}
			}
			if (attacking == true) {
				if (spriteNum == 1) {image = attackRight1;}			
				if (spriteNum == 2) {image = attackRight2;}
		...
		if (tookDamage == true) {
			g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.3f)); // Visual effect to invincible state
		...
		// Note, screenX and Y are used to draw the image from the top left corner of the screen. This must be fixed. 
